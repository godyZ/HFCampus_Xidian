...
- (void)dismissPopupViewControllerWithanimationType:(MJPopupViewAnimation)animationType
{
    ...
    if (self.mjDelegate && [self.mjDelegate respondsToSelector:@selector(MJDidDismissPopupView)]) {
        [self.mjDelegate MJDidDismissPopupView];
    }
}
...
- (void)presentPopupFadeView:(UIView *)popupView point:(CGPoint)point   //这个是自定义的 point为起始点
{
    UIView *sourceView = [self topView];
    ...
    [self fadeViewIn:popupView sourceView:sourceView overlayView:overlayView point:point];
}
...
- (void)fadeViewIn:(UIView*)popupView sourceView:(UIView*)sourceView overlayView:(UIView*)overlayView
{
    ...
    CGSize popupSize = popupView.bounds.size;
    CGRect popupEndRect = CGRectMake((sourceSize.width - popupSize.width) / 2, 
                                     (sourceSize.height - popupSize.height) / 2,  //微调位置
                                     popupSize.width, 
                                     popupSize.height);
...
- (void)fadeViewIn:(UIView*)popupView sourceView:(UIView*)sourceView overlayView:(UIView*)overlayView point:(CGPoint)point
{
    UIView *backgroundView = [overlayView viewWithTag:kMJBackgroundViewTag];
    // Generating Start and Stop Positions
    //CGSize sourceSize = sourceView.bounds.size;
    CGSize popupSize = popupView.bounds.size;
    CGRect popupEndRect = CGRectMake(point.x,
                                     point.y, //(sourceSize.height - popupSize.height) / 2,  //微调位置
                                     popupSize.width,
                                     popupSize.height);
